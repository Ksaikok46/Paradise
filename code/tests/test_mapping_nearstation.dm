...
/datum/game_test/maptest_mapping_nearstation_test

/datum/game_test/maptest_mapping_nearstation_test/Run()
	...
	for(var/station_z in levels_by_trait(STATION_LEVEL))
		...
		for(var/turf/station_turf as anything in turfs_to_check)
			...
			for(var/atom/movable/thing_on_the_turf as anything in station_turf.contents) //Find an item on the turf, this can help the mapper identify the turf more easily when combined with the exact coords.
				...
				has_thing = TRUE
				break
			if(!has_thing && !isspaceturf(station_turf) && !istype(station_turf, /turf/simulated/openspace)) //In case it's just a turf without an area
				if(istype(station_turf, /turf/simulated/floor/engine/hull/ceiling))
