...
/datum/controller/subsystem/processing/dcs/proc/rotate_decals(datum/source, old_dir, new_dir)
	...
	for(var/result in resulting_decals_params)
		source.AddElement(/datum/element/decal, result["icon"], result["icon_state"], result["dir"], PLANE_TO_TRUE(result["plane"]), result["layer"], result["alpha"], result["color"], result["cleanable"], result["desc"])
...
/datum/element/decal/Attach(atom/target, _icon, _icon_state, _dir, _plane = FLOAT_PLANE, _layer = FLOAT_LAYER, _alpha = 255, _color, _smoothing, _cleanable = CLEAN_GOD, _description, mutable_appearance/_pic)
	...
	if(isitem(target))
		INVOKE_ASYNC(target, TYPE_PROC_REF(/obj/item, update_equipped_item), TRUE)
	if(_dir)
...
/datum/element/decal/Detach(atom/source)
	UnregisterSignal(source, list(COMSIG_ATOM_DIR_CHANGE, COMSIG_COMPONENT_CLEAN_ACT, COMSIG_ATOM_EXAMINE, COMSIG_ATOM_UPDATE_OVERLAYS, COMSIG_TURF_ON_SHUTTLE_MOVE, COMSIG_ATOM_DECALS_ROTATING))
