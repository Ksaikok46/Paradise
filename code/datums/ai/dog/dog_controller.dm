...
/datum/ai_controller/dog/TryPossessPawn(atom/new_pawn)
	...
	RegisterSignals(new_pawn, list(COMSIG_MOB_DEATH, COMSIG_QDELETING), PROC_REF(on_death))
...
/datum/ai_controller/dog/UnpossessPawn(destroy)
	...
	if(carried_item)
		pawn.visible_message(span_warning("[DECLENT_RU_CAP(pawn, NOMINATIVE)] выплевывает [carried_item.declent_ru(ACCUSATIVE)]."))
...
/datum/ai_controller/dog/proc/on_attack_hand(datum/source, mob/living/user)
	...
	if(user.a_intent == INTENT_HARM)
		...
		if(blackboard[BB_SIMPLE_CARRY_ITEM] && !current_movement_target && friends[WEAKREF(user)])
			var/obj/item/carried_item = blackboard[BB_SIMPLE_CARRY_ITEM]
			pawn.visible_message(span_warning("[DECLENT_RU_CAP(pawn, NOMINATIVE)] бросает [carried_item.declent_ru(ACCUSATIVE)] к ногам [user]!"))
...
/datum/ai_controller/dog/proc/befriend(mob/living/new_friend)
	...
	if(in_range(pawn, new_friend))
		new_friend.visible_message(
			span_notice("<b>[DECLENT_RU_CAP(pawn, NOMINATIVE)]</b> дружелюбно облизывает [new_friend]!"),
			span_notice("[DECLENT_RU_CAP(pawn, NOMINATIVE)] дружелюбно облизывает вас!"))
...
/datum/ai_controller/dog/proc/on_examined(datum/source, mob/user, list/examine_text)
	...
	var/obj/item/carried_item = blackboard[BB_SIMPLE_CARRY_ITEM]
	if(carried_item)
		examine_text += span_notice("В [GEND_HIS_HER(pawn)] пасти находится [carried_item.declent_ru(NOMINATIVE)].")
...
/datum/ai_controller/dog/proc/on_death(mob/living/ol_yeller)
	...
	ol_yeller.visible_message(span_warning("[DECLENT_RU_CAP(ol_yeller, NOMINATIVE)] отпускает [carried_item.declent_ru(ACCUSATIVE)] из своей пасти..."))
...
/datum/ai_controller/dog/proc/set_command_mode(mob/commander, command)
	...
	switch(command)
		...
		if(COMMAND_HEEL)
			pawn.visible_message(span_notice("[DECLENT_RU_CAP(pawn, NOMINATIVE)] садится на задние лапы, ожидая новой команды от [commander]."))
		...
		if(COMMAND_FETCH)
			pawn.visible_message(span_notice("[DECLENT_RU_CAP(pawn, NOMINATIVE)] в предвкушении готовится дернуться с места!"))
			blackboard[BB_DOG_ORDER_MODE] = DOG_COMMAND_FETCH
		// attack: harass whoever the commander points to
		if(COMMAND_ATTACK)
			pawn.visible_message(span_warning("[DECLENT_RU_CAP(pawn, NOMINATIVE)] настораживается и начинает агрессивно рычать.")) // imagine getting intimidated by a corgi
	...
	switch(blackboard[BB_DOG_ORDER_MODE])
		if(DOG_COMMAND_FETCH)
			...
			if(pointed_item.obj_flags & ABSTRACT)
				return
			pawn.visible_message(span_notice("[DECLENT_RU_CAP(pawn, NOMINATIVE)] следует за указаниями [pointing_friend] и радостно гафкает!"))
		...
		if(DOG_COMMAND_ATTACK)
			pawn.visible_message(span_notice("[DECLENT_RU_CAP(pawn, NOMINATIVE)] следует за указаниями [pointing_friend] и агрессивно рычит!"))
			current_movement_target = pointed_movable
